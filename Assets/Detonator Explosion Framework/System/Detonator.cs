...
public class Detonator : MonoBehaviour
{
	...
	*/
				...
				if (!_firstComponentUpdate) {
						...
						foreach (DetonatorComponent component in components) {
								if (component.detonatorControlled) {
										...
										component.velocity = new Vector3 (directionVector.x * component.startVelocity.x * s + sdir.x, directionVector.y * component.startVelocity.y * s + sdir.y, component.startVelocity.z * s + sdir.z);
		...
		public void Explode ()
		{
				this.Explode (Vector4.one * strength);
		}

		public void Explode (Vector4 explosionLength)
		{
				_lastExplosionTime = Time.time;
		
				foreach (DetonatorComponent component in components) {
						UpdateComponents ();
						component.Explode (explosionLength);
				}
				
				foreach (Detonator detonator in gameObject.transform.GetComponentsInChildren<Detonator> ()) {
						if (detonator != this) {
		...
		public static Material DefaultFireballAMaterial ()
		{
				Debug.Log (defaultFireballAMaterial);
