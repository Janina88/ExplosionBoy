	...
	public void UpdateFireballB()
{
	...
	override public void Explode(Vector4 explosionLength)
	{
		...
		if (on)
		{
			...
			UpdateFireballB();
			UpdateFireShadow();
			if (drawFireballA)	_fireballAEmitter.Explode(explosionLength);
			if (drawFireballB) _fireballBEmitter.Explode(explosionLength);
			if (drawFireShadow)	_fireShadowEmitter.Explode(explosionLength);
		}
	}
