...
public class DetonatorBurstEmitter : DetonatorComponent
{
	...
	private Vector4 explosionLength;
	...
	*/
	...
	void Update () 
	{
		...
		Vector3 emitterPos = _particleEmitter.transform.position;

		for(int i = 0; i < particles.Length; i++) {
			Vector3 particlePos = particles [i].position;
			if (particlePos.y >= emitterPos.y + explosionLength.w) {
				particles[i].energy = 0f;
			}
			else if (particlePos.x >= emitterPos.x + explosionLength.x) {
				particles[i].energy = 0f;
			}
			else if (particlePos.x <= emitterPos.x - explosionLength.y) {
				particles[i].energy = 0f;
			}
			else if (particlePos.y <= emitterPos.y - explosionLength.z) {
				particles[i].energy = 0f;
			}
		}
	...
	override public void Explode(Vector4 explosionLength)
	{
